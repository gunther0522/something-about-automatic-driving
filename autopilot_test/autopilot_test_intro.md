...
场景库建立的目的是验证智能驾驶系统的软硬件功能和性能。

场景库特点：`无限性` `扩展性` `批量化` `自动化`

按照一定格式定义的场景库接口可以将场景库与测试系统、被测对象解耦，避免场景重复构建。

自动驾驶测试场景基于数据库接口要素：`路` `人` `车` `交通环境`

```
1. 路：路网，高精度结构化路网
```
路网是场景库基础。通常路网为高精度路网或高精度地图。
常用的路网接口定义形式：`OpenDRIVE格式` `RoadXML格式` `百度Apollo基于OpenDRIVE的标准做的部分改动和扩展`

OpenDRIVE：一种开源道路网逻辑描述文件格式，结构层级最多可为8层


```
2. 人：驾驶人接口定义
```
不同类型的驾驶人对相同场景执行驾驶操作的主观行为。
通常可设置为`默认` `舒适` `迟疑` `激进`等类型。
可调整参数：`期望速度` `期望加速度` `期望减速度` `转弯行为` `遵守限速意愿` `行车距离控制` `车道保持` `速度保持` `变道频率` `变道速度` `紧急工况反应速度`
